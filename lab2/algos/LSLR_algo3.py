...
class LSLRAlgo3(LSLROptimiser):
    ...
    def __init__(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        self.eta = 10/vals.max()
        self.alpha = 0.2
        self.iter = 0
        self.gamma = int(np.sqrt(self.X.shape[0]))
    ...
    def lr2(self) -> float:
        return self.lr()/(1+self.alpha*self.iter)

    def step(self, params: np.ndarray) -> np.ndarray:
        ## TODO Implement the step method
        self.iter += 1
        return params - self.lr2()*self.stoch_grad2(params, self.gamma)
